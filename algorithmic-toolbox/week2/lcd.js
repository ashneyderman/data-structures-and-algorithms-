...
function readArrayOrExit(line) {
  let arrStr = line.toString().split(' ')
  
  if(arrStr.length != 2) {
    console.log(`too many or too few possible numbers expecting 2 parsed ${arrStr.length}`)
    process.exit()
  }
  
  a = new BigNumber(arrStr[0])
  b = new BigNumber(arrStr[1])
  
  if(a.val() == 'Invalid Number' || b.val() == 'Invalid Number') {
    console.log('Invalid Number', line.toString())
    process.exit()
  }
}
...
function readLine (line) {
  if (line !== "\n") {
    readArrayOrExit(line)
    let result = (new BigNumber(a)).multiply(new BigNumber(b)).divide(gcd(new BigNumber(a), new BigNumber(b)))
